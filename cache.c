...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
    ...
    b_index = addr_word % 4; // where 4 is the block size in words
    ...
    if (slot->valid && (slot->tag == tag)) {
        ...
        if (csp->block_size == 1) {
            // since each slot can contain multiple words
            // we isolate this case based on block size
            data = slot->block[b_index];
        }
        else {
            // goes to memory and gets values for block sizes > 1
            for(int i = 0; i < csp->block_size; i++) {
                slot->block[i] = *(uint32_t*)((b_base * 4) + i);
                // type casts it into a word
            }
        }
    }
        ...
        if (slot_invalid) {
            ...
            for(int i = 1; i < 4; i++) {
                if (csp->slots[set_base + i].timestamp<slot->timestamp) {
                    // if condition met, update oldest slot
                    slot = &(csp->slots[set_base +i]);
                }
            }
